...
st.markdown("<p style='text-align:center; color:#666; letter-spacing:3px; font-family:Rajdhani;'>UTC+5 TIME SYNCED | 3-MODE ENGINE</p>", unsafe_allow_html=True)
...
# Complete requested currency pairs dictionary with proper flag formatting and Yahoo Finance symbols
CURRENCY_PAIRS = {
...
# Custom Timer/Expiry sequence requested: 1-5m, then 5s-55s increments, then 6-15m
def get_custom_timer_options():
    options = []
    # 1 min to 5 min first
    for m in range(1, 6):
        options.append(f"{m} Minute" if m == 1 else f"{m} Minutes")
    # 5s to 55s in 5s increments
...
analysis_mode = st.selectbox(
    ...
    [
        ...
        "3️⃣ Live Market Data + AI Predictor (1m - 15m Expiry)"
    ]
)
st.markdown("---")
...
if analysis_mode == "1️⃣ Screenshot Vision Analyzer (Auto 1-Min Standard)":
    ...
    if uploaded_file:
        ...
        if st.button("🚀 Analyze & Get Next-Minute Signal", key="btn_m1"):
            with st.spinner("AI is analyzing candlesticks, indicators & calculating entry timing (UTC+5)..."):
                try:
                    buffered = io.BytesIO()
                    ...
                    4. Calculate the precise entry target for the NEXT upcoming candle/minute. Account for execution safety by targeting an entry exactly 2 seconds before the new minute starts (e.g., HH:MM:58).
...
# --- MODE 2: Screenshot Vision Analyzer with Manual Custom Timer ---
    ...
    if uploaded_file:
        ...
        if st.button("🚀 Analyze with Custom Timer", key="btn_m2"):
            with st.spinner(f"Analyzing chart for {selected_timer} expiry (UTC+5)..."):
                try:
                    buffered = io.BytesIO()
                    if img.mode in ("RGBA", "P"):
                        img = img.convert("RGB")
...
# --- MODE 3: Live Market Data + AI Predictor (1m - 15m Expiry) ---
else:
    ...
    with col_p:
        ...
        selected_pair_key = selected_display.split(" ")[1] # Extracts "EUR/JPY" etc.
    with col_t:
        # Expiry options filtered strictly from 1 minute to 15 minutes as requested
    ...
    if st.button("🚀 Fetch Live Market Data & Generate AI Signal", key="btn_m3"):
        with st.spinner(f"Pulling live candles for {selected_pair_key} ({selected_expiry}) via yfinance and passing to OpenRouter AI..."):
            try:
                ...
                reason = "Momentum analysis supports continuation for the selected expiry."
